...
BASE_FOLDER = "/Users/shankarsharma/Desktop/GQL/stepzen_gql_api/src/soap"
...
def init_stepzen_workspace():
    ...
    if not os.path.exists(os.path.join(WORKSPACE_DIR, ".stepzen")):
...
def generate_graphql_from_wsdl(wsdl_url):
    """Generate StepZen GraphQL schema + SDL directives from WSDL"""
    print(f"[INFO] Parsing WSDL: {wsdl_url}")
    # Safe Zeep transport: timeout + optional SSL verification off
    session = Session()
    session.verify = True  # Set False only if SSL issues
    transport = Transport(session=session, timeout=10)  # 10 seconds timeout

    try:
        client = Client(wsdl_url, transport=transport)
    except Exception as e:
        raise RuntimeError(f"[ERROR] Zeep failed to load WSDL: {e}")
    graphql_types = []
    graphql_queries = []

    # StepZen SDL directives will be embedded in schema
    sdl_blocks = []

    for service in client.wsdl.services.values():
        for port in service.ports.values():
            binding = port.binding
            soap_address = port.binding_options.get("address") # SOAP endpoint URL
            if not soap_address:
                raise RuntimeError(f"Could not determine SOAP endpoint for port: {port.name}")
            print(f"[INFO] Processing service: {service.name}, port: {port.name}, address: {soap_address}")
            for operation in binding._operations.values():
                op_name = operation.name

                # Input args
                args = []
                soap_body = f"<{op_name} xmlns=\"{binding.nsmap.get(None)}\">"
                if operation.input:
                    for part in operation.input.body.type.elements:
                        arg_name = part[0]
                        arg_type = "String"
                        if "int" in str(part[1]):
                            arg_type = "Int"
                        elif "float" in str(part[1]):
                            arg_type = "Float"
                        elif "boolean" in str(part[1]):
                            arg_type = "Boolean"

                        args.append(f"{arg_name}: {arg_type}!")
                        soap_body += f"<{arg_name}>{{{{args.{arg_name}}}}}</{arg_name}>"

                soap_body += f"</{op_name}>"

                # Return type
                return_type = f"{op_name}Response"
                graphql_queries.append(f"{op_name}({', '.join(args)}): {return_type}")
                print(f"[INFO] Generated query: {op_name} with args: {args} and return type: {return_type}")
                graphql_types.append(f"""
                type {return_type} {{
                result: String
                }}
                """)

                #StepZen SDL directive block
                sdl_block = f"""
                    @rest(endpoint: "{soap_address}"
                          method: "POST"
                          headers: [
                ...
                sdl_blocks.append(sdl_block)

    # Combine into schema
    # Ensure all parts are joined properly
    ...
    # File paths
    schema_path = os.path.join(WORKSPACE_DIR, "schema.graphql")
    index_path = os.path.join("/Users/shankarsharma/Desktop/GQL/stepzen_gql_api", "index.graphql")

     # Write schema.graphql with everything
    with open(schema_path, "w") as f:
        f.write("\n".join(graphql_types) + "\n")
        f.write("type Query {\n  " + "\n  ".join(graphql_queries) + "\n}\n")
        f.write("\n".join(sdl_blocks))

    # Write index.graphql importing schema.graphql
    index_content = """  # index.graphql
                         schema @sdl(files: ["src/soap/schema.graphql"]) {
                         query: Query
                         }
                    """
    ...
    print(f"[INFO] Generated schema.graphql and index.graphql in {WORKSPACE_DIR}")
...
{indent(postbody_xml, "        ")}
    ...
    with open(os.path.join(WORKSPACE_DIR, "schema.graphql"), "w") as f:
        f.write(schema_body)
...
'''
    with open(os.path.join(WORKSPACE_DIR, "index.graphql"), "w") as f:
        f.write(index_body)
...
if __name__ == "__main__":
    WORKSPACE_DIR = "/Users/shankarsharma/Desktop/GQL/stepzen_gql_api/src/soap"
    ...
    generate_schema_and_index(WSDL_URL, WORKSPACE_DIR)
    deploy_stepzen()
